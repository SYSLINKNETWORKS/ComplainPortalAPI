...
--select * from t_ddc
--alter table t_ddc add titm_id_free int
--alter table t_ddc add constraint FK_TDDC_TITMIDFREE foreign key (titm_id_free) references t_itm(titm_id)

--alter table t_ddc add ddc_despa float,ddc_despb float,ddc_ret float,ddc_usola float,ddc_usolb float

--Insert
alter proc ins_t_ddc(@ddc_qty float,@ddc_stdsiz float,@titm_id int,@itmqty_id int,@mdc_id int,@sca_id int,@lot_id int,@ddc_bqty float,@ddc_schamt float,@titm_id_free int,@ddc_freeqty float,@ddc_bat varchar(100),@ddc_mandat datetime,@ddc_expdat datetime,
@ddc_despa float,@ddc_despb float,@ddc_ret float,@ddc_usola float,@ddc_usolb float)
as
declare
@ddc_id int,
@wh_id int,
...
begin
	...
	set @stk_dat=(select mdc_dat from t_mdc where mdc_id=@mdc_id)
			...
			if (@titm_id_free =0)
				begin
					set @titm_id_free=null
				end
			insert into t_ddc(ddc_id,ddc_qty,mdc_id,titm_id,itmqty_id,ddc_st,ddc_stdsiz,sca_id,lot_id,ddc_bqty,ddc_schamt,titm_id_free,ddc_freeqty,ddc_bat,ddc_mandat,ddc_expdat,ddc_despa,ddc_despb,ddc_ret,ddc_usola,ddc_usolb)
					values(@ddc_id,@ddc_qty,@mdc_id,@titm_id,@itmqty_id,0,@ddc_stdsiz,@sca_id,@lot_id,@ddc_bqty,@ddc_schamt,@titm_id_free,@ddc_freeqty,@ddc_bat,@ddc_mandat,@ddc_expdat,@ddc_despa,@ddc_despb,@ddc_ret,@ddc_usola,@ddc_usolb)
		...
		--set @stk_qty=(isnull((select round(sum(stk_qty),4) from m_stk where stk_frm in ('t_itm','GRN','ITN') and stk_dat<=@stk_dat_old and itm_id=@titm_id and m_yr_id=@m_yr_id and wh_id=@wh_id),0))
		...
		--set @stk_amt=(isnull((select round(sum(stk_qty*stk_rat),4) from m_stk where stk_frm in ('t_itm','GRN','ITN') and stk_dat<=@stk_dat_old and itm_id=@titm_id and m_yr_id=@m_yr_id and wh_id=@wh_id),0))
		...
		--		set @stk_rat=0
		--	end
			set @stk_rat=(select titmrat_rrat from t_itmrat where titm_id=@titm_id and titmrat_dat=(select MAX(titmrat_dat) from t_itmrat titmrat where t_itmrat.titm_id=titmrat.titm_id and titmrat.titmrat_dat <=@stk_dat))
