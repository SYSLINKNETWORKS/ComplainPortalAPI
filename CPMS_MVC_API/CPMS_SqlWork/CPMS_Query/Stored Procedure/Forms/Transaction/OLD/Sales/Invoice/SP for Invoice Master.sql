...
--alter table t_minv add minv_bdisper float,minv_bdisamt float
--update t_minv set minv_bdisper=0,minv_bdisamt=0

--alter table t_minv add minv_frecon char(1)
--update t_minv set minv_frecon ='+'

--alter table t_minv add minv_commamt float
--update t_minv set minv_commamt =0

--Insert
alter proc ins_t_minv(@com_id char(2),@br_id char(3),@m_yr_id char(2),@minv_dat datetime,@minv_taxdat datetime,@minv_typ char(1),@dc_no varchar(100),@minv_rat float,@cur_id int,@cus_id int,@minv_rmk varchar(250),@minv_cktax bit,@minv_taxamt float,@minv_gstamt float,@minv_fedamt float,@minv_taxnamt float,@minv_amt float,@minv_disamt float,@minv_bdisper float,@minv_bdisamt float,@minv_frecon char(1),@minv_freamt float,@minv_othamt float,@minv_commamt float,@minv_namt float,@minv_can bit,@log_act char(1),@usr_id int,@log_ip varchar(100),@log_frmnam varchar(1024),@log_oldval text,@log_newval text,@minv_no_out int output,@minv_taxid_out int output,@minv_id_out int output)
as
declare
@minv_id int,
@minv_no int,
...
begin
	...
	if (@minv_typ='O')
		begin
			set @minv_id=isnull((select min(minv_id) from t_minv where minv_typ='O')-1,-1)
			set @minv_no=isnull((select min(minv_no) from t_minv where minv_typ='O')-1,-1)
		end
	else
		begin
			set @minv_id=isnull((select max(minv_id) from t_minv)+1,1)
			set @minv_no=isnull((select max(minv_no) from t_minv)+1,1)
		end
	...
	insert into t_minv(minv_no,minv_id,minv_taxid,minv_dat,minv_taxdat,minv_typ,cus_id,minv_act,minv_rmk,minv_rat,cur_id,minv_cktax,minv_taxnamt,minv_gstamt,minv_fedamt,minv_taxamt,minv_amt,minv_disamt,minv_bdisper,minv_bdisamt,minv_taxdisamt,minv_frecon,minv_freamt,minv_othamt,minv_commamt,minv_namt,m_yr_id,mdc_id,com_id,br_id,minv_can,log_act,log_dat,usr_id,log_ip)
			values(@minv_no,@minv_id,@minv_taxid,@minv_dat,@minv_taxdat,@minv_typ,@cus_id,0,@minv_rmk,@minv_rat,@cur_id,@minv_cktax,@minv_taxnamt,@minv_gstamt,@minv_fedamt,@minv_taxnamt,@minv_amt,@minv_disamt,@minv_bdisper,@minv_bdisamt,@minv_taxdisamt,@minv_frecon,@minv_freamt,@minv_othamt,@minv_commamt,@minv_namt,@m_yr_id,@dc_no,@com_id,@br_id,@minv_can,@log_act,@log_dat,@usr_id,@log_ip)
...
begin
	...
	update t_minv set minv_taxid=@minv_taxid,minv_dat=@minv_dat,minv_taxdat=@minv_taxdat,minv_typ=@minv_typ,minv_rmk=@minv_rmk,cus_id=@cus_id,minv_rat=@minv_rat,cur_id=@cur_id,minv_cktax=@minv_cktax,minv_taxamt=@minv_taxamt,minv_gstamt=@minv_gstamt,minv_taxnamt=@minv_taxnamt,minv_fedamt=@minv_fedamt,minv_amt=@minv_amt,minv_disamt=@minv_disamt,minv_bdisper=@minv_bdisper,minv_bdisamt=@minv_bdisamt,minv_taxdisamt=@minv_taxdisamt,minv_frecon=@minv_frecon,minv_freamt=@minv_freamt,minv_othamt=@minv_othamt,minv_commamt=@minv_commamt,minv_namt=@minv_namt,mdc_id=@dc_no,minv_can=@minv_can,log_act=@log_act,log_dat=@log_dat,usr_id=@usr_id,log_ip=@log_ip
