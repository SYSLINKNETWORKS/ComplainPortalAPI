...
--			update t_minv set minv_act=1,minv_typ='S' where minv_id=@minv_id
			update t_dinv set dinv_st=1 where minv_id=@minv_id
			--For Opening Invoice
			update t_minv set minv_act=1 where minv_id=@minv_id and minv_typ='O'
			--For Others
			update t_minv set minv_act=1,minv_typ='S' where minv_id=@minv_id and minv_typ<>'O'
		end
...
begin
	...
	update t_dinv set dinv_st=0 where minv_id in (select minv_id from t_drec where mrec_id=@mrec_id)
	--For Opening Invoice
	update t_minv set minv_act=0,minv_typ='O' where minv_id in (select minv_id from t_drec where mrec_id=@mrec_id) and minv_typ='O'
	--For Other Invoice
	update t_minv set minv_act=0,minv_typ='U' where minv_id in (select minv_id from t_drec where mrec_id=@mrec_id) and minv_typ!='O'
	delete from t_drec where mrec_id =@mrec_id
