...
begin
	...
	if (@mdc_typ='O')
		begin
			set @mdc_id=isnull((select min(mdc_id) from t_mdc where mdc_typ='O')-1,-1)
			set @mdc_no=isnull((select min(mdc_no) from t_mdc where mdc_typ='O')-1,-1)
		end
	else
		begin
			set @mdc_id=isnull((select max(mdc_id) from t_mdc)+1,1)
			set @mdc_no=isnull((select max(mdc_no) from t_mdc)+1,1)
		end
	...
	insert into t_mdc(com_id,br_id,m_yr_id,mdc_id,mdc_no,mdc_dat,mdc_typ,mdc_act,mdc_auth,mso_id,wh_id,mdc_rmk,mdc_vno,mdc_drv,mdc_deptime,mdc_ckdel,mdc_delnam,mdc_delpho,mdc_deladd,carr_id,mdc_bltyno,mdc_bltydat,log_act,log_dat,usr_id,log_ip)
				values(@com_id,@br_id,@m_yr_id,@mdc_id,@mdc_no,@mdc_dat,@mdc_typ,0,@mdc_auth,@mso_id,@wh_id,@mdc_rmk,@mdc_vno,@mdc_drv,@mdc_deptime,@mdc_ckdel,@mdc_delnam,@mdc_delpho,@mdc_deladd,@carr_id,@mdc_bltyno,@mdc_bltydat,@log_act,@log_dat,@usr_id,@log_ip)
...
begin
	...
	delete from t_mdc where mdc_id=@mdc_id
--Audit
exec sp_ins_log @log_frmnam,@log_act,@usr_id,@log_ip,@log_oldval,@log_newval,@log_dat
