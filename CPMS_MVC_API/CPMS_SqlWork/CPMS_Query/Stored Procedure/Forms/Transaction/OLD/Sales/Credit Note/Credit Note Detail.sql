...
--alter table t_dcn add res_id int
--alter table t_dcn add ck_amtadj bit
--alter table t_dcn add res_id int
--alter table t_dcn drop column ck_amtadj 
--alter table t_dcn add dcn_ckamtadj bit
--alter table t_dcn DROP COLUMN dcn_ckitmret 

 --alter table t_dcn add dcn_salcom float

--Inserth
alter proc ins_t_dcn(@dcn_qty float,@titm_id int,@dcn_rat float,@dcn_amt float,@dcn_disper float,@dcn_disamt float,@dcn_namt float,@mcn_id int,@row_id int)
as
declare
@com_id char(2),
@br_id char(3),
...
@cus_nam varchar(250),
@stk_des varchar(250),
@itmqty_id int,@dcn_stdsiz float,
@dcn_gstper float,@dcn_gstamt float,@dcn_fedper float,@dcn_fedamt float,
@res_id int,@mdc_id int,@sca_id int,
@dcn_salcom float,
@minv_id int
begin
	set @dcn_gstper=0
	...
	set @dcn_salcom =(select @dcn_qty*(dinv_salcom/dinv_qty) from t_dinv where minv_id=@minv_id and titm_id=@titm_id )
	...
	insert into t_dcn(dcn_id,dcn_qty,dcn_rat,dcn_amt,dcn_disper,dcn_disamt,dcn_gstper,dcn_gstamt,dcn_fedper,dcn_fedamt,dcn_namt,mcn_id,titm_id,itmqty_id,mdc_id,dcn_stdsiz,sca_id,res_id,dcn_salcom)
			values(@dcn_id,@dcn_qty,@dcn_rat,@dcn_amt,@dcn_disper,@dcn_disamt,@dcn_gstper,@dcn_gstamt,@dcn_fedper,@dcn_fedamt,@dcn_namt,@mcn_id,@titm_id,@itmqty_id,@mdc_id,@dcn_stdsiz,@sca_id,@res_id,@dcn_salcom)
		...
		if (@stk_qty<>0)
			...
			--Stock	
			set @stk_des='CN# ' +rtrim(cast(@mcn_no as varchar(100))) + ' Invoice # '+rtrim(cast(@minv_id as varchar(100)))+ ' Customer: ' +@cus_nam	
