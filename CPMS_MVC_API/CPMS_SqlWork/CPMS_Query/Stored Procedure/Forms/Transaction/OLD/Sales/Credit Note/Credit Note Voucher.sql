...
--select * from t_mcn
--select * from t_dvch where mvch_no=(select mvch_no from t_mcn)
--exec sp_voucher_cn 1
...
--GL Voucher
		...
		set @mcn_frecon =(select mcn_frecon from t_mcn where mcn_id=@mcn_id)
		...
		set @emppro_id=(select emppro_id from t_mcn where mcn_id=@mcn_id)
...
--Detail Voucher
					...
					--Debit Voucher				
					set @nar='Credit Note # ' + rtrim(cast(@mcn_no as char(100))) +' Customer Name : ' +@cus_nam 
					set @row_id=@row_id+1
					set @acc_no=(select sal_ret_acc from m_sys) 					
					set @titm_amt=@mcn_amt--+@mcn_bdisamt+@mcn_disamt-@mcn_salcom
				...
				--Salesman Commission Payable
					...
					--Credit
					set @nar='Credit Note # ' + rtrim(cast(@mcn_no as char(100))) 
					set @row_id=@row_id+1
