...
--alter table m_sys add bdis_acc int
--update m_sys set bdis_acc=(select acc_no from gl_m_acc where acc_id='04001013')

--alter table m_sys add salcom_acc int
--update m_sys set salcom_acc=(select acc_no from gl_m_acc where acc_id='02003001001')

--alter table m_sys add salcom_exp_acc int
--update m_sys set salcom_exp_acc=(select acc_no from gl_m_acc where acc_id='05003019')
...
--GL Voucher
		...
		set @minv_bdisamt=(select round(minv_bdisamt,0) from t_minv where minv_id=@minv_id)
		set @minv_frecon=(select minv_frecon from t_minv where minv_id=@minv_id)
		...
		set @minv_commamt =(select ROUND(minv_commamt,0) from t_minv where minv_id=@minv_id)
		...
		set @minv_salcom=isnull((select sum(dinv_salcom) from t_dinv where minv_id=@minv_id),0)
		...
		set @minv_typ=(select minv_typ from t_minv where minv_id=@minv_id)
		if (@minv_typ='O')
			begin
				set @typ_id='06'
			end
		...
		if (@mvch_no is null)
			begin
				--Master Voucher			
				exec ins_t_mvch @com_id,@br_id,@m_yr_id,@minv_dat ,@minv_dat ,'','01',@typ_id,'Y','C' ,@minv_rmk,'S','',@minv_dat,0,0,@cur_id,@minv_rat,@minv_can,0,'','','','','','',@mvch_no_out=@mvch_no output
...
--Detail Voucher
					set @nar='Invoice # ' + rtrim(cast(@minv_no as char(100)))
--Debit
					set @row_id=@row_id+1
					...
					if (@minv_bdisamt>0)
						begin
							...
							exec ins_t_dvch @com_id,@br_id,@mvch_no,@row_id,@acc_no,@nar,@minv_bdisamt,0
						end
				...
				--Salesman Commission Payable
					if (@minv_commamt>0)
						begin
							set @row_id=@row_id+1
							set @acc_no=(select salcom_exp_acc from m_sys)
							exec ins_t_dvch @com_id,@br_id,@mvch_no,@row_id,@acc_no,@nar,0,@minv_commamt
						end
				...
				--Other Income
					...
					if (@minv_freamt>0)
						begin
							...
							if (@minv_frecon='+')
								...
								begin
									exec ins_t_dvch @com_id,@br_id,@mvch_no,@row_id,@acc_no,@nar,@minv_freamt,0								
								end
						end
					--GST AMT
					if (@minv_gstamt>0)
					...
					set @titm_amt=@minv_namt-isnull(@minv_othamt,0)-isnull(@minv_gstamt,0)-isnull(@minv_fedamt,0)+isnull(@minv_disamt,0)+isnull(@minv_bdisamt,0)-isnull(@minv_salcom,0)-isnull(@minv_commamt,0)
					if (@minv_frecon='+')
						begin
							set @titm_amt=@titm_amt-@minv_freamt
						end
					else if (@minv_frecon='-')
						begin
							set @titm_amt=@titm_amt+@minv_freamt
