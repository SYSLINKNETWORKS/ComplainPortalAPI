...
--alter table t_dinv add dinv_bdisper float,dinv_bdisamt float
--update t_dinv set dinv_bdisper=0,dinv_bdisamt=0

--alter table t_dinv drop column dinv_bdisper,dinv_bdisamt

--alter table t_dinv add dinv_salcom float

--alter table t_dinv add dinv_despa float,dinv_despb float,dinv_ret float,dinv_usola float,dinv_usolb float
...
@dinv_despa float,@dinv_despb float,@dinv_ret float,@dinv_usola float,@dinv_usolb float)
as
declare
@dinv_id int,
@qty int,
...
begin
	...
	set @emppro_id=(select emppro_id from t_mso where mso_id=@mso_id)
	set @minv_dat=(select minv_dat from t_minv where minv_id=@minv_id)
	set @dinv_salcom =(select @dinv_qty*titmsalcom_comm from t_itmsalcom
						where t_itmsalcom.titm_id=@titm_id and t_itmsalcom.emppro_id =@emppro_id 
						and t_itmsalcom.titmsalcom_dat=(select MAX(titmsalcom_dat) from t_itmsalcom titmsalcom where t_itmsalcom.titm_id=titmsalcom.titm_id and t_itmsalcom.emppro_id =titmsalcom.emppro_id and titmsalcom.titmsalcom_dat<=@minv_dat))
	...
	set @dinv_id=(select max(dinv_id)+1 from t_dinv)
		...
		if (@dinv_tax =0)
			...
			if (@free_titm_id=0)
				begin
					set @free_titm_id =null
				end
			insert into t_dinv(dinv_id,dinv_tax,dinv_qty,dinv_taxqty,dinv_rat,dinv_taxrat,dinv_amt,dinv_taxamt,dinv_disper,dinv_disamt,dinv_taxdisamt,dinv_othamt,dinv_gstper,dinv_gstamt,dinv_fedper,dinv_fedamt,dinv_namt,dinv_taxnamt,minv_id,titm_id,itmqty_id,mdc_id,sca_id,dinv_stdsiz,dinv_bqty,dinv_schamt,free_titm_id,dinv_freeqty,dinv_bat,dinv_mandat,dinv_expdat,dinv_salcom,dinv_despa,dinv_despb,dinv_ret,dinv_usola,dinv_usolb)
