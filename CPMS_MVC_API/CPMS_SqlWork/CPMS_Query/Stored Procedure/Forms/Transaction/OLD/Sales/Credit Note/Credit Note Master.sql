...
--select emppro_id from t_mcn

--alter table t_mcn add emppro_id int
--alter table t_mcn add constraint FK_TMCN_EMPPROID foreign key (emppro_id) references m_emppro(emppro_id)
--alter table t_mcn add mcn_frecon char(1)

--Insert
alter proc ins_t_mcn(@com_id char(2),@br_id char(3),@m_yr_id char(2),@cus_id int,@wh_id int,@emppro_macid int,@mcn_dat datetime,@mcn_rmk varchar(250),@mcn_amt float,@mcn_bdisper float,@mcn_bdisamt float,@mcn_disper float,@mcn_disamt float,@mcn_frecon char(1),@mcn_freamt float,@mcn_namt float,@mcn_typ char(1),@mcn_can bit,@mcn_cash float,@mso_id int,@aud_frmnam varchar(250),@aud_des varchar(1000),@usr_id int,@aud_ip char(100),@mcn_no_out int output,@mcn_id_out int output)
as
declare
@mcn_id int,
@mcn_no int,
...
begin
	...
	insert into t_mcn(com_id,br_id,m_yr_id,mcn_id,mcn_no,mcn_taxid,mcn_dat,mcn_rmk,mcn_typ,mcn_cktax,mcn_gstamt,mcn_fedamt,mcn_amt,mcn_disper,mcn_disamt,mcn_gamt,mcn_frecon,mcn_freamt,mcn_namt,mcn_act,mcn_can,wh_id,cus_id,cur_id,mcn_currat,ins_usr_id,ins_dat,mcn_ckitmret,mcn_cash,mcn_bdisper,mcn_bdisamt,emppro_id)
			values(@com_id,@br_id,@m_yr_id,@mcn_id,@mcn_no,@mcn_taxid,@mcn_dat,@mcn_rmk,@mcn_typ,@mcn_cktax,@mcn_gstamt,@mcn_fedamt,@mcn_amt,@mcn_disper,@mcn_disamt,@mcn_gamt,@mcn_frecon,@mcn_freamt,@mcn_namt,0,@mcn_can,@wh_id,@cus_id,@cur_id,@mcn_currat,@usr_id,GETDATE(),@mcn_ckitmret,@mcn_cash,@mcn_bdisper,@mcn_bdisamt,@emppro_id)
...
go
--Update
alter proc upd_t_mcn(@com_id char(2),@br_id char(3),@m_yr_id char(2),@cus_id int,@wh_id int,@emppro_macid int,@mcn_id int,@mcn_dat datetime,@mcn_rmk varchar(250),@mcn_amt float,@mcn_disper float,@mcn_disamt float,@mcn_frecon char(1),@mcn_freamt float,@mcn_namt float,@mso_id int,@mcn_typ char(1),@mcn_can bit,@mcn_cash float,@mcn_bdisper float,@mcn_bdisamt float,@aud_frmnam varchar(250),@aud_des varchar(1000),@usr_id int,@aud_ip char(100))
as
declare
@aud_act char(10),
@cur_id int,
...
begin
	...
	update t_mcn set mcn_taxid=@mcn_taxid,mcn_dat=@mcn_dat,mcn_rmk=@mcn_rmk,mcn_typ=@mcn_typ,cur_id=@cur_id,mcn_currat=@mcn_currat,mcn_cktax=@mcn_cktax,mcn_gstamt=@mcn_gstamt,mcn_fedamt=@mcn_fedamt,mcn_amt=@mcn_amt,mcn_disper=@mcn_disper,mcn_disamt=@mcn_disamt,mcn_gamt=@mcn_gamt,mcn_frecon=@mcn_frecon,mcn_freamt=@mcn_freamt,mcn_namt=@mcn_namt ,mcn_can=@mcn_can,wh_id=@wh_id,cus_id=@cus_id,upd_usr_id=@usr_id,upd_dat=GETDATE(),mcn_ckitmret=@mcn_ckitmret ,mcn_cash=@mcn_cash,mcn_bdisper=@mcn_bdisper,mcn_bdisamt=@mcn_bdisamt,emppro_id=@emppro_id
	 where mcn_id=@mcn_id
--Audit
	exec sp_ins_aud1 @com_id,@br_id,@aud_frmnam,@aud_des,@usr_id,@aud_act,@aud_ip
