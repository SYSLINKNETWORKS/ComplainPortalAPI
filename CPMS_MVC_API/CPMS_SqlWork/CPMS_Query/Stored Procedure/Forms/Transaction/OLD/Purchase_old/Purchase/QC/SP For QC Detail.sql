...
--alter table t_dqc add dqc_qtyapp float
--alter table t_dqc add dqc_ar int
--alter table t_dqc add dqc_redat datetime
--select * from t_dqc


alter proc ins_t_dqc(@ck_mandat bit,@dqc_man datetime,@ck_dat bit,@dqc_exp datetime,@dqc_stdsiz float,@dqc_qty float,@dqc_acc float,@dqc_nqty float,@dqc_rat float,@dqc_amt float,@mqc_id int,@mgrn_id int,@titm_id int,@itmqty_id int,@sca_id int,@m_yr_id char(2),@dqc_bat varchar(100),@dqc_typ char(1),@dqc_rmk varchar(1000),@coun_id int,@man_id int,@dqc_qtyapp float,@dqc_redat datetime)
as
declare
@dqc_id int,
@qc_qty int,
...
begin
	...
	insert into t_dqc
		(dqc_id,dqc_man,dqc_exp,dqc_stdsiz,dqc_qty,dqc_acc,dqc_nqty,dqc_rat,dqc_trat,dqc_amt,dqc_tamt,mqc_id,titm_id,itmqty_id,sca_id,m_yr_id,dqc_bat,dqc_typ,dqc_rmk,coun_id,man_id,dqc_qtyapp,dqc_ar,dqc_redat)
		values
		(@dqc_id,@dqc_man,@dqc_exp,@dqc_stdsiz,@dqc_qty,@dqc_acc,@dqc_nqty,@dqc_rat,@dqc_trat,@dqc_amt,@dqc_tamt,@mqc_id,@titm_id,@itmqty_id,@sca_id,@m_yr_id,@dqc_bat,@dqc_typ,@dqc_rmk,@coun_id,@man_id,@dqc_qtyapp,@dqc_ar,@dqc_redat)
	...
	if (@dqc_rat=0)
		begin
			...
			set @dqc_amt=@dqc_qty*@dqc_rat
		end
	--Stock	
	set @stk_qty=round(@dqc_qty/@sca_met,2)
	set @stk_acc=round(@dqc_acc/@sca_met,2)
	...
	set @grn_qty=(select sum(dgrn_qty) from t_dgrn where mgrn_id=@mgrn_id and titm_id=@titm_id)
	set @qc_qty=(select sum(dqc_qty) from t_mqc inner join t_dqc on t_mqc.mqc_id=t_dqc.mqc_id where mgrn_id=@mgrn_id and titm_id=@titm_id)
			if ((@grn_qty-@qc_qty)<=0)
				begin
	...
	set @grn_qty=(select distinct sum(dgrn_qty) from t_dgrn where mgrn_id=@mgrn_id )
	set @qc_qty=(select distinct sum(dqc_qty) from  t_mqc inner join t_dqc on t_mqc.mqc_id=t_dqc.mqc_id where mgrn_id=@mgrn_id )
