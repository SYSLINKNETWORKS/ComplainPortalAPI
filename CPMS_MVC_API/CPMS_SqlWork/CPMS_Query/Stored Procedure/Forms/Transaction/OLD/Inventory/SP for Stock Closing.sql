...
--select * from m_stk where m_yr_id='02' and stk_frm='t_itm' and itm_id in (select titm_id from t_itm where itm_id=5)
...
begin
	...
	if (@itm_cat='F')
		begin
			...
			select @t_id,titm_id,round(SUM(isnull(stk_qty,0)),4),case when sum(stk_qty)>0 then round(AVG(isnull(stk_trat,0)),4) else 0 end,'U','R','t_itm',@stk_dat,isnull(wh_id,(select top 1 wh_id from m_itm where itm_id=t_itm.itm_id)),t_itm.bd_id,@m_yr_id,titm_exp,@cur_id,1,0,m_stk.stk_bat,m_stk.mso_id,0,@stk_dat,0,0 
			from t_itm 
			left join m_stk 
			...
			group by titm_id,wh_id,m_Stk.bd_id,t_itm.itm_id,t_itm.bd_id ,m_stk.stk_bat,m_Stk.mso_id,titm_exp
		end
	else if(@itm_cat ='G')
		begin
			insert into m_stk (t_id,itm_id,stk_qty,stk_rat,stk_typ,stk_st,stk_frm,stk_dat,wh_id,bd_id,m_yr_id,titm_exp,cur_id,stk_currat,stk_qtyacc,stk_bat,mso_id,stk_batst,titm_maf,stk_actrat,stk_foh)
			select @t_id,titm_id,round(SUM(isnull(stk_qty,0)),4),case when sum(stk_qty)>0 then round(AVG(isnull(stk_trat,0)),4) else 0 end,'U','R','t_itm',@stk_dat,isnull(wh_id,(select top 1 wh_id from m_itm where itm_id=t_itm.itm_id)),t_itm.bd_id,@m_yr_id, null ,@cur_id,1,0,null,null,0,null,0,0 
			from t_itm 
			inner join m_itmsub
			...
			group by titm_id,wh_id,m_Stk.bd_id,t_itm.itm_id,t_itm.bd_id 
