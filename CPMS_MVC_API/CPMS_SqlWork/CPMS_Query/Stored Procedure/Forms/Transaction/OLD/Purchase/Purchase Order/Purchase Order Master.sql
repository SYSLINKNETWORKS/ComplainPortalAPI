...
--alter table t_mpo add mpo_auth bit
--update t_mpo set mpo_auth=0
--alter table t_mpo add mpo_credays float
--update t_mpo set mpo_credays=90
--alter table t_mpo add ind_id int
--alter table t_mpo add constraint FK_TMPO_INDID foreign key (ind_id) references m_ind(ind_id)


--Insert
alter proc ins_t_mpo(@com_id char(2),@br_id char(3),@mpo_dat datetime,@mpo_ddat datetime,@mpo_credays float,@mpo_typ char(1),@mpo_rat float,@mpo_amt float,@m_yr_id char(2),@mpo_act bit,@mpo_rmk varchar(250),@mpr_id int,@cur_id int,@sup_id int,@lc_nam varchar(250),@mpo_purtyp char(2),@ind_id int,@mpo_auth bit,@log_act char(1),@usr_id int,@log_ip varchar(100),@log_frmnam varchar(1024),@log_oldval text,@log_newval text,@mpo_id_out int output)
as
declare
@mpo_id int,
@log_dat datetime
begin
	...
	insert into t_mpo(com_id,br_id,mpo_id,mpo_dat,mpo_ddat,mpo_credays,mpo_typ,m_yr_id ,mpo_rat,mpr_id,mpo_amt,sup_id,cur_id,mpo_act,mpo_rmk,lc_nam,mpo_purtyp,mpo_auth,log_act,log_dat,usr_id,log_ip,ind_id)
			values(@com_id,@br_id,@mpo_id,@mpo_dat,@mpo_ddat,@mpo_credays,@mpo_typ,@m_yr_id,@mpo_rat,@mpr_id,@mpo_amt,@sup_id,@cur_id,@mpo_act,@mpo_rmk,@lc_nam ,@mpo_purtyp,@mpo_auth,@log_act,@log_dat,@usr_id,@log_ip,@ind_id)
...
begin
	...
	update t_mpo set mpo_dat=@mpo_dat,mpo_ddat=@mpo_ddat,mpo_credays=@mpo_credays,mpo_rat=@mpo_rat,mpo_amt=@mpo_amt,sup_id=@sup_id,cur_id=@cur_id,mpo_act=@mpo_act,mpo_rmk=@mpo_rmk,lc_nam=@lc_nam,mpo_purtyp=@mpo_purtyp,ind_id=@ind_id,mpo_auth=@mpo_auth,log_act=@log_act,log_dat=@log_dat,usr_id=@usr_id,log_ip=@log_ip where mpo_id=@mpo_id
