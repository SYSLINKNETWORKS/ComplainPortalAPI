...
--alter table t_dpo add dpo_appqty float

--Insert
alter proc ins_t_dpo(@ck_mandat bit,@dpo_man datetime,@ck_dat bit,@dpo_exp datetime,@dpo_stdsiz float,@dpo_qty float,@dpo_rat float,@dpo_amt float,@dpo_rmk varchar(250),@titm_id int,@itmqty_id int,@sca_id int,@titm_img bit ,@mpo_id int,@m_yr_id char(2),@dpo_appqty float)
as
declare
@mpr_id int,
@dpo_id int,
...
begin
	...
	insert into t_dpo(dpo_id,dpo_man,dpo_exp,dpo_stdsiz,dpo_qty,dpo_rat,dpo_trat,dpo_amt,dpo_tamt,dpo_rmk,mpo_id,titm_id,itmqty_id,sca_id,dpo_st,m_yr_id,titm_img,dpo_appqty)
			values(@dpo_id,@dpo_man,@dpo_exp,@dpo_stdsiz,@dpo_qty,@dpo_rat,@dpo_trat,@dpo_amt,@dpo_tamt,@dpo_rmk,@mpo_id,@titm_id,@itmqty_id,@sca_id,0,@m_yr_id,@titm_img,@dpo_appqty)

	set @pr_qty=(select sum(dpr_appqty) from t_dpr where mpr_id=@mpr_id and titm_id=@titm_id)
	...
	set @pr_qty=(select distinct sum(dpr_appqty) from t_dpr where mpr_id=@mpr_id)
