...
--alter table t_mqc add emppro_id int
--alter table t_mqc add constraint FK_MQC_EMPPROID foreign key (emppro_id) references m_emppro(emppro_id)
--update t_mqc set emppro_id=(select emppro_id from m_emppro where emppro_macid=t_mqc.emppro_macid)
--alter table t_mqc drop column emppro_macid

--alter table t_mqc add log_act char(1),log_dat datetime,usr_id int,log_ip varchar(100)

--alter table t_mqc add mqc_auth bit
--update t_mqc set mqc_auth=0
--alter table t_mqc add mqc_cktax bit default 0
--alter table t_dqc add dqc_tax bit default 0



--Insert
alter proc ins_t_mqc(@com_id char(2),@br_id char(3),@m_yr_id char(2),@mqc_dat datetime,@mqc_typ char(1),@mqc_nw float,@sca_id int,@mqc_act bit,@mqc_auth bit,@sup_dc varchar(100),@sup_dcdat datetime,@mqc_kepz varchar(100),@mqc_rmk varchar(250),@mqc_rat float,@mqc_amt float,@mgrn_id int,@wh_id int,@mqc_idold varchar(1000),@emppro_macid int,@log_act char(1),@usr_id int,@log_ip varchar(100),@log_frmnam varchar(1024),@log_oldval text,@log_newval text,@mqc_id_out int output)
as
declare
@mqc_id int,
@cur_id int,
...
begin
	...
	insert into t_mqc(com_id,br_id,mqc_id,mqc_dat,mqc_typ,mgrn_id,m_yr_id,sup_dc,sup_dcdat,mqc_kepz,mqc_rmk,mqc_rat,mqc_amt,cur_id,mqc_act,mqc_nw,sca_id,wh_id,mqc_idold,lot_id,emppro_id,mqc_auth,log_act,log_dat,usr_id,log_ip)
			values(@com_id,@br_id,@mqc_id,@mqc_dat,@mqc_typ,@mgrn_id,@m_yr_id,@sup_dc,@sup_dcdat,@mqc_kepz,@mqc_rmk,@mqc_rat,@mqc_amt,@cur_id,@mqc_act,@mqc_nw,@sca_id,@wh_id,@mqc_idold,@lot_id,@emppro_id,@mqc_auth,@log_act,@log_dat,@usr_id,@log_ip)
...
begin
	...
	update t_mqc set mqc_dat=@mqc_dat,sup_dc=@sup_dc,sup_dcdat=@sup_dcdat,mqc_kepz=@mqc_kepz,mqc_rmk=@mqc_rmk,mqc_rat=@mqc_rat,mqc_amt=@mqc_amt,cur_id=@cur_id,mqc_act=@mqc_act,mqc_auth =@mqc_auth,wh_id=@wh_id,mqc_nw=@mqc_nw,sca_id=@sca_id,log_act=@log_act,log_dat=@log_dat,usr_id=@usr_id,log_ip=@log_ip,emppro_id=@emppro_id where mqc_id=@mqc_id
