...
--alter table t_dgrn add dgrn_appqty float
--update t_dgrn set dgrn_appqty=0
--alter table t_dgrn add dgrn_rmk varchar(1000)


alter proc ins_t_dgrn(@ck_mandat bit,@dgrn_man datetime,@ck_dat bit,@dgrn_exp datetime,@dgrn_stdsiz float,@dgrn_qty float,@dgrn_acc float,@dgrn_nqty float,@mgrn_id int,@mpo_id int,@titm_id int,@itmqty_id int,@sca_id int,@m_yr_id char(2),@dgrn_bat varchar(100),@coun_id int,@man_id int,@dgrn_appqty float,@dgrn_rmk varchar(1000))
as
declare
@dgrn_id int,
@grn_qty int,
...
begin
	...
	set @dgrn_rat=(select dpo_rat from t_dpo where mpo_id=@mpo_id and titm_id=@titm_id)
	set @dgrn_amt=@dgrn_rat*@dgrn_qty
	...
	insert into t_dgrn
		(dgrn_id,dgrn_man,dgrn_exp,dgrn_stdsiz,dgrn_qty,dgrn_acc,dgrn_nqty,dgrn_rat,dgrn_trat,dgrn_amt,dgrn_tamt,mgrn_id,titm_id,itmqty_id,sca_id,m_yr_id,dgrn_bat,coun_id,man_id,dgrn_appqty,dgrn_rmk)
		values
		(@dgrn_id,@dgrn_man,@dgrn_exp,@dgrn_stdsiz,@dgrn_qty,@dgrn_acc,@dgrn_nqty,@dgrn_rat,@dgrn_trat,@dgrn_amt,@dgrn_tamt,@mgrn_id,@titm_id,@itmqty_id,@sca_id,@m_yr_id,@dgrn_bat,@coun_id,@man_id,@dgrn_appqty,@dgrn_rmk)
	...
	if (@dgrn_rat=0)
		begin
			...
			set @dgrn_amt=@dgrn_appqty*@dgrn_rat
		end
	--Stock	
	set @stk_qty=@dgrn_appqty
	set @stk_acc=@dgrn_acc
	...
	set @po_qty=(select sum(dpo_appqty) from t_dpo where mpo_id=@mpo_id and titm_id=@titm_id)
	set @grn_qty=(select sum(dgrn_appqty) from t_mgrn inner join t_dgrn on t_mgrn.mgrn_id=t_dgrn.mgrn_id where mpo_id=@mpo_id and titm_id=@titm_id)
			if ((@po_qty-@grn_qty)<=0)
				begin
	...
	set @po_qty=(select distinct sum(dpo_appqty) from t_dpo where mpo_id=@mpo_id )
