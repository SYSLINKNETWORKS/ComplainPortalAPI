...
as
begin
	update t_mvch set mvch_tax=1 where com_id=@com_id and mvch_no=@mvch_no
	...
	declare  mvch1  cursor for			
		...
		OPEN mvch1
			...
			INTO @mvch_oldvoucherno,@yr_id ,@mvch_dat ,@mvch_tdat ,@mvch_pto ,@dpt_id ,@typ_id ,@mvch_app ,@mvch_cb ,@mvch_ref ,@mvch_typ ,@mvch_chq ,@mvch_chqdat ,@mvch_po,@mvch_chqst ,@cur_id ,@mvch_rat ,@mvch_can ,@aud_frmnam ,@aud_des ,@usr_id ,@aud_ip
				...
				BEGIN
					exec zsons_tax.dbo.ins_t_mvch @com_id ,@br_id ,@yr_id ,@mvch_dat ,@mvch_tdat ,@mvch_pto ,@dpt_id ,@typ_id ,@mvch_app ,@mvch_cb ,@mvch_ref ,@mvch_typ ,@mvch_chq ,@mvch_chqdat ,@mvch_po,@mvch_chqst ,@cur_id ,@mvch_rat ,@mvch_can ,@aud_frmnam ,@aud_des ,@usr_id ,@aud_ip ,@mvch_oldvoucherno ,'',@mvch_no_out =@mvch_no_new output
						...
						declare  dvch1  cursor for			
							...
							OPEN dvch1
								...
								INTO @dvch_row,@acc_no,@dvch_nar,@dvch_dr_famt,@dvch_cr_famt
									WHILE @@FETCH_STATUS = 0
									BEGIN
										exec zsons_tax.dbo.ins_t_dvch @com_id ,@br_id ,@mvch_no_new ,@dvch_row ,@acc_no ,@dvch_nar ,@dvch_dr_famt ,@dvch_cr_famt 
...
as
begin
	update t_mvch set mvch_tax=0 where com_id=@com_id and mvch_no in 
	(
...
declare @mvch_no int,@yr_id char(2),@aud_frmnam varchar(250),@aud_des varchar(1000),@usr_id int,@aud_ip char(100)
	declare  mvch_del  cursor for			
							select t_mvch.br_id,t_mvch.mvch_no,t_mvch.yr_id,'','','','' from zsons_tax.dbo.t_mvch inner join zsons_tax.dbo.t_dvch on t_mvch.com_id=t_dvch.com_id and t_mvch.br_id=t_dvch.br_id and t_mvch.mvch_no=t_dvch.mvch_no inner join gl_m_acc on t_mvch.com_id=gl_m_acc.com_id and t_dvch.acc_no=gl_m_acc.acc_no where t_mvch.com_id=@com_id and left(acc_id,len(@acc_id))=@acc_id and mvch_dt between @dt1 and @dt2
							OPEN mvch_del
								FETCH NEXT FROM mvch_del
								INTO @br_id,@mvch_no ,@yr_id ,@aud_frmnam ,@aud_des ,@usr_id ,@aud_ip
									WHILE @@FETCH_STATUS = 0
									BEGIN
										exec zsons_tax.dbo.del_t_vch @com_id ,@br_id ,@yr_id ,@mvch_no ,'','','',''
