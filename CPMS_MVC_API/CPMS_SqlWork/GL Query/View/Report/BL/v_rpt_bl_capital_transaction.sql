--select * from v_rpt_bl_capital_transaction

alter view v_rpt_bl_capital_transaction
as
--Capital
	select t_mvch.com_id,t_mvch.br_id,t_mvch.yr_id,gl_m_acc.acc_id,'Partners Capital' as [acc_nam],'Capital and Liablities' as [acc_mnam],'C' as [Tag],1 as [MTag],1 AS [GMTAG],
...
union
--Profit / Loss
--	select com_id,br_id,yr_id,gl_m_acc.acc_id,'Accumulated Profit & Loss Account' as [acc_nam],'Capital and Liablities' as [acc_mnam],'PL' as [Tag],1 as [MTag],1 AS [GMTAG],
--		case when mvch_dt 
--			between '07/01/'+(select yr_str_yy from gl_m_yr where yr_ac='Y') and '07/31/'+(select yr_str_yy from gl_m_yr where yr_ac='Y') then sum(dvch_dr_amt-dvch_cr_amt) else 0 end as [Jul],
--		case when mvch_dt 
--			between '08/01/'+(select yr_str_yy from gl_m_yr where yr_ac='Y') and '08/31/'+(select yr_str_yy from gl_m_yr where yr_ac='Y') then sum(dvch_dr_amt-dvch_cr_amt) else 0 end as [Aug],
...
--	from gl_m_acc
--	inner join gl_m_acc
--	on gl_m_acc.acc_id=gl_m_acc.acc_id
--	where 
...
--Liablities against Assets
	...
	where 
	gl_m_acc.acc_cid in ('02003001','02003002') and mvch_app='Y'
	group by t_mvch.com_id,t_mvch.br_id,t_mvch.yr_id,gl_m_acc.acc_id,t_mvch.mvch_dt
...
union
--Current Liabilities
	select t_mvch.com_id,t_mvch.br_id,t_mvch.yr_id,gl_m_acc.acc_id,'Creditors, A/Exp. & O/Liab.' as [acc_nam],'Current Liabilities' as [acc_mnam],'CL' as [Tag],2 as [MTag],1 as [GMTAG],
		case when t_mvch.mvch_dt 
			between '07/01/'+(select yr_str_yy from gl_m_yr where yr_ac='Y') and '07/31/'+(select yr_str_yy from gl_m_yr where yr_ac='Y') then sum(dvch_dr_amt-dvch_cr_amt) else 0 end as [Jul],
		case when t_mvch.mvch_dt 
			between '08/01/'+(select yr_str_yy from gl_m_yr where yr_ac='Y') and '08/31/'+(select yr_str_yy from gl_m_yr where yr_ac='Y') then sum(dvch_dr_amt-dvch_cr_amt) else 0 end as [Aug],
	...
	where 
		left(gl_m_acc.acc_cid,8) in ('02003003','02003004') and mvch_app='Y'
	group by t_mvch.com_id,t_mvch.br_id,t_mvch.yr_id,gl_m_acc.acc_id,t_mvch.mvch_dt
	 
	 
union
--Taxation
	select t_mvch.com_id,t_mvch.br_id,t_mvch.yr_id,gl_m_acc.acc_id,'Taxation' as [acc_nam],'Current Liabilities' as [acc_mnam],'TAX' as [Tag],2 as [MTag],1 as [GMTAG],
		case when t_mvch.mvch_dt 
			between '07/01/'+(select yr_str_yy from gl_m_yr where yr_ac='Y') and '07/31/'+(select yr_str_yy from gl_m_yr where yr_ac='Y') then sum(dvch_dr_amt-dvch_cr_amt) else 0 end as [Jul],
		case when t_mvch.mvch_dt 
			between '08/01/'+(select yr_str_yy from gl_m_yr where yr_ac='Y') and '08/31/'+(select yr_str_yy from gl_m_yr where yr_ac='Y') then sum(dvch_dr_amt-dvch_cr_amt) else 0 end as [Aug],
	...
	where 
			left(gl_m_acc.acc_cid,8) in ('02003005') and mvch_app='Y'
	group by t_mvch.com_id,t_mvch.br_id,t_mvch.yr_id,gl_m_acc.acc_id,t_mvch.mvch_dt
